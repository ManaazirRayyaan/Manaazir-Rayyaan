...
import { initialInquiryValues, inquirySchema, type InquiryFormValues } from "@/lib/contact";
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!parsed.success) {
      ...
      for (const issue of parsed.error.issues) {
        const path = issue.path[0] as keyof InquiryFormValues | undefined;
  ...
  return (
    <div className="glass-panel rounded-[1.25rem] p-5 sm:rounded-[1.5rem] sm:p-8">
      <div className="max-w-2xl">
        <p className="text-[11px] uppercase tracking-[0.18em] text-[var(--muted)] sm:text-sm sm:tracking-[0.28em]">Contact</p>
        <h2 className="mt-3 text-2xl font-semibold tracking-tight sm:text-3xl">Tell me what you need, and I’ll reply with the next step.</h2>
        <p className="mt-3 text-sm leading-7 text-[var(--muted)]">
          Keep it simple. Share your idea, your current problem, or the result you want to achieve.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="mt-8 space-y-5">
        ...
        <div>
          ...
          <input id="name" name="name" value={values.name} onChange={handleChange} className={inputClass(Boolean(errors.name))} placeholder="Your name" />
        ...
        <div>
          ...
          <input id="email" name="email" type="email" value={values.email} onChange={handleChange} className={inputClass(Boolean(errors.email))} placeholder="you@example.com" />
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="Tell me about your project, the problem you need solved, or what you want built."
          />
        ...
        {status.type !== "idle" ? (
          <p className={status.type === "success" ? "text-sm text-emerald-600" : "text-sm text-red-500"}>{status.message}</p>
        ) : null}
      </form>
    </div>
  );
